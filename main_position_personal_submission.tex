...
Both labels are claim-level; neither is a verdict for the complete response.}
\label{fig:flip}
\end{figure}
...
\textbf{``Summarize this medical report''}\hfill
\colorbox{clrB!12}{\strut $\sigma_p=0,\ \kappa_p=0$}\\[-1pt]
\emph{Contract:} $O_p=$ provided report; $\Gamma_p=\varnothing$; $\rho_p=$ factual assertion.\par\smallskip
\begin{tabularx}{\linewidth}{@{}YP{0.15\linewidth}P{0.26\linewidth}@{}}
``The patient has diabetes'' (not in the report) & \textcolor{clrH}{\bfseries \Hall} & unsupported by $O_p$ and not licensed\\
\end{tabularx}
\end{minipage}}
...
\begin{minipage}{0.94\linewidth}
\textbf{``Explain photosynthesis like a poet, but keep the science accurate''}\hfill
\colorbox{clrLD!12}{\strut $\sigma_p=2,\ \kappa_p=0$}\\[-1pt]
\emph{Contract:} $O_p=$ scientific references; $\Gamma_p=\varnothing$; $\rho_p=$ factual assertion.\par\smallskip
\begin{tabularx}{\linewidth}{@{}YP{0.15\linewidth}P{0.26\linewidth}@{}}
``Chlorophyll catches sunlight, fueling the plant's sugar-making work'' & \textcolor{clrLD}{\bfseries \SUP+\SV} & supported claim in marked form\\
``Plants make sugar from moonlight'' & \textcolor{clrH}{\bfseries \Hall} & contradicted claim; $\sigma_p=2$ does not license it\\
\end{tabularx}
\end{minipage}}
...
\begin{minipage}{0.94\linewidth}
\textbf{``List possible causes; mark anything unestablished''}\hfill
\colorbox{clrP!12}{\strut $\sigma_p=0,\ \kappa_p=1$}\\[-1pt]
\emph{Contract:} $O_p=$ clinical evidence; $\Gamma_p=$ possible diagnoses; $\rho_p=$ explicit uncertainty marker.\par\smallskip
\begin{tabularx}{\linewidth}{@{}YP{0.15\linewidth}P{0.26\linewidth}@{}}
``This could be early Lyme disease, but testing is required'' & \textcolor{clrLD}{\bfseries \LD} & unknown, in scope, and properly hedged\\
``This is early Lyme disease'' & \textcolor{clrH}{\bfseries \Hall} & required hedge is missing\\
\end{tabularx}
\end{minipage}}
...
\begin{minipage}{0.94\linewidth}
\textbf{``Write a story set in 1920s Paris''}\hfill
\colorbox{clrO!12}{\strut $\sigma_p=1,\ \kappa_p=2$}\\[-1pt]
\emph{Contract:} $O_p=$ prompt and historical constraints; $\Gamma_p=$ fictional world; $\rho_p=$ declared fictional frame.\par\smallskip
\begin{tabularx}{\linewidth}{@{}YP{0.15\linewidth}P{0.26\linewidth}@{}}
``The fictional caf\'e Verdier opened at dawn'' & \textcolor{clrLD}{\bfseries \LD} & invention inside $\Gamma_p$\\
A fabricated statement about a real named person, presented as biography & \textcolor{clrH}{\bfseries \Hall} & real-world claim outside $\Gamma_p$\\
\end{tabularx}
\end{minipage}}
\end{table*}
...
We also did not identify a protocol that jointly evaluates $\sigma_p$-conditioned span typing, \SV\ placement, extracted claim sets, and downstream claim-level labels.
...
\caption{\textbf{Worked cases illustrating contract-dependent claim labels.}
Each contrast may change one or more components.
\SUP, \LD, and \Hall\ are claim-level labels; \SV\ is an auxiliary span flag.}
\label{tab:cases}
...
The framework motivates four empirical priorities.
...
Research should compare transparent aggregation rules---for example, the proportion of \Hall\ claims, a worst-case rule, or severity-weighted scoring---under different truth contracts.
Evaluation should report the underlying claim-label distribution as well as any aggregate score, because different aggregation rules can rank the same responses differently.
We leave the design and validation of an optimal contract-aware $R$ to future work.
...
The framework assigns claim-level labels but does not validate a response-level aggregation rule; defining $R(y\mid p)$ is an explicit limitation and research direction.

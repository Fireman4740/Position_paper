...
The contract specifies reference evidence $O_p$, requested style $\sigma_p$, and a content-permission policy $K_p=(\kappa_p,\Gamma_p,\rho_p)$.
Its permission level $\kappa_p\in\{0,1,2\}$ distinguishes factual-only tasks, limited divergence, and in-frame invention; $\Gamma_p$ and $\rho_p$ specify where and how divergence is allowed.
...
These are claim-level labels: a complete response may contain several of them.
Current methods often aggregate claim results, but no single rule is known to be optimal across truth contracts; contract-aware response-level assessment remains an open problem.
...
These labels apply to individual claims.
Assessing a complete response requires a separate aggregation step over its claim labels and other properties, such as coverage, usefulness, severity, and style compliance.
This paper does not claim an optimal response-level aggregation rule.
...
The wording is fixed, but the prompt establishes a different discourse frame, so the two occurrences do not express the same contextualized claim.
...
This constructed example starts from the same claim wording in a hypothetical model response.
Under the historical-QA prompt, it is a real-world assertion unsupported by $O_p$ and receives \Hall.
Under the fiction-writing prompt, it is an in-frame invention permitted by $K_p$ and receives \LD.
Thus, a claim extracted from a response can be labelled differently when the prompt changes the truth contract.
Strictly, the discourse frame changes the contextualized claim; the figure therefore shows contract dependence, not a contradiction about one fixed proposition.
...
For $\Gamma_p$, consider a medical brainstorming task that permits possible diagnoses but not invented drug doses; the diagnosis space is inside the scope, whereas treatment instructions are outside it.
...
Each colored card fixes one truth contract $\TC(p)=(O_p,\sigma_p,K_p)$, with $K_p=(\kappa_p,\Gamma_p,\rho_p)$, and places its examples one below another.
\SUP, \Hall, and \LD\ are claim-level labels; \SV\ is an auxiliary span label.
The table does not assign one verdict to the complete response: response-level aggregation is a separate open problem.}
\label{tab:contracts}
\setlength{\fboxsep}{5pt}
\fcolorbox{clrB}{clrB!3}{%
\begin{minipage}{0.94\linewidth}
\textbf{Prompt:} ``Summarize this medical report''\hfill
\colorbox{clrB!12}{\strut $\sigma_p=0,\ \kappa_p=0$}\\[-1pt]
\textbf{Truth contract:} $O_p=$ provided report; $\Gamma_p=\varnothing$; $\rho_p=$ factual assertion.\par\smallskip
\begin{tabularx}{\linewidth}{@{}YP{0.15\linewidth}P{0.26\linewidth}@{}}
...
\begin{minipage}{0.94\linewidth}
\textbf{Prompt:} ``Explain photosynthesis like a poet, but keep the science accurate''\\[-1pt]
\colorbox{clrLD!12}{\strut $\sigma_p=2,\ \kappa_p=0$}\\[-1pt]
\textbf{Truth contract:} $O_p=$ scientific references; $\Gamma_p=\varnothing$; $\rho_p=$ factual assertion.\par\smallskip
\begin{tabularx}{\linewidth}{@{}YP{0.15\linewidth}P{0.26\linewidth}@{}}
\textbf{Response claim:} ``Chlorophyll catches sunlight, fueling the plant's sugar-making work'' & \textcolor{clrLD}{\bfseries \SUP+\SV} & supported claim in marked form\\
...
\begin{minipage}{0.94\linewidth}
\textbf{Prompt:} ``List possible causes; mark anything unestablished''\\[-1pt]
\colorbox{clrP!12}{\strut $\sigma_p=0,\ \kappa_p=1$}\\[-1pt]
...
\begin{minipage}{0.94\linewidth}
\textbf{Prompt:} ``Write a story set in 1920s Paris''\hfill
\colorbox{clrO!12}{\strut $\sigma_p=1,\ \kappa_p=2$}\\[-1pt]
...
\textbf{Response claim:} A fabricated statement about a real named person, presented as biography & \textcolor{clrH}{\bfseries \Hall} & real-world claim outside $\Gamma_p$\\
\end{tabularx}
\end{minipage}}
\end{table*}
...
The cases are kept in prose because each contrast requires an explanation that a compact summary table would repeat without adding a new result.
...
Because this paper does not define an optimal $R$, its formal results should not be read as a complete response-scoring method.
...
These boundaries define the next validation steps while leaving the paper's central distinction---between evidential support, content permission, and requested style---fully testable.
...
These are claim-level decisions.
...
\appendixsection{Additional Diagnostics and Response-Level Aggregation}\label{app:diagnostics}

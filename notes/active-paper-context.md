...
- Preserve result numbers unless explicitly re-running or revising the analysis.
- Do not cite keys that are missing from `references.bib`.
- Mark unsupported claims with `\citetodo{...}` or `\claimtodo{...}`.
